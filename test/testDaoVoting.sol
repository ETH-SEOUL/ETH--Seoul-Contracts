...
import {DeploySoulbound} from "../src/DeploySoulbound.sol";
...
contract TestDAOVoting is Test {
    ...
    DeploySoulbound deploySoulbound;
    ...
    function setUp() public {
        ...
        timeLock = new TimeLock(MIN_DELAY, proposors, executors);
        ...
        //Let's deploy the deploySoulbound contract (the contract that we want to be governed)
        deploySoulbound = new DeploySoulbound(address(0));

        // Let's now transfer the ownership of the deploySoulbound to the timeLock contract
        deploySoulbound.transferOwnership(address(timeLock));
        ...
        targets[0] = address(deploySoulbound);

        bytes memory makeDeploySoulboundCall = abi.encodeWithSelector(
            DeploySoulbound.deploySoulboundToken.selector,
            "0x564aAa1a81A83876A4ce398dBC8110c7B7376e74",
            "test",
            0
        );

        bytes[] memory calldatas = new bytes[](1);
        calldatas[0] = makeDeploySoulboundCall;

        string
            memory proposalDescription = "Testing the deployment of a new Soulbound Token";
        ...
        assertTrue(
            governorContract.state(proposalID) == IGovernor.ProposalState.Active
        );
    }
    ...
    function test_voting() public {
        ...
        uint256 governanceTokenBalance = governanceToken.balanceOf(
            address(this)
        );
        ...
        uint8 support = 0; // We are voting For
        string memory reason = "I am for this standard";
        ...
        vm.roll(block.timestamp + VOTING_PERIOD + 100);
